...
///////////////////////////////////////////////////////////////
static bool GetVersionInfo(const TCHAR *pInfPath, const TCHAR *pKey, TCHAR **ppValue, bool showErrors)
{
  if (!pInfPath)
    return FALSE;
  ...
  if (!SetupGetInfInformation(pInfPath, INFINFO_INF_NAME_IS_ABSOLUTE, NULL, 0, &size)) {
    if (showErrors)
      ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupGetInfInformation() for %s"),pInfPath);
    return FALSE;
  }
  ...
  if (pInformation) {
    ...
    ShowLastError(MB_OK|MB_ICONSTOP, _T("LocalAlloc(%lu)"),(unsigned long)size);
    return FALSE;
  }

  if (!SetupQueryInfVersionInformation(pInformation, 0, pKey, NULL, 0, &size)) {
    if (showErrors)
      ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupQueryInfVersionInformation(%s) for %s"),pKey, pInfPath);
    LocalFree(pInformation);
    return FALSE;
  }

  *ppValue = (TCHAR *)LocalAlloc(LPTR, size*sizeof(*ppValue[0]));
  ...
  if (*ppValue) {
    ...
    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    ShowLastError(MB_OK|MB_ICONSTOP, _T("LocalAlloc(%lu)"),(unsigned long)(size*sizeof(*ppValue[0])));
    LocalFree(pInformation);
    return FALSE;
  }
...
///////////////////////////////////////////////////////////////
static bool Open(const TCHAR *pInfPath, HINF *phInf, bool showErrors)
{
    ...
    *phInf = SetupOpenInfFile(pInfPath, NULL, INF_STYLE_WIN4, &errLine);
    ...
    if (*phInf == INVALID_HANDLE_VALUE) {
      ...
      res = ShowLastError(MB_RETRYCANCEL,
                          _T("SetupOpenInfFile(%s) on line %u"),
                          pInfPath, errLine);
    }
  } while (res == IDRETRY);
    ...
    DWORD lenFilePath)
{
  ...
  if (!pNearPath) {
    if (DWORD(lstrlen(pFileName)) >= lenFilePath) {
      ShowError(MB_OK|MB_ICONSTOP, ERROR_BUFFER_OVERFLOW, _T("lstrlen(%s) >= %lu"),
  ...
  if (!res) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("GetFullPathName(%s)"),pNearPath);
    return FALSE;
  }

  if (res >= lenFilePath) {
    ShowError(MB_OK|MB_ICONSTOP, ERROR_BUFFER_OVERFLOW, _T("GetFullPathName(%s)"),pNearPath);
    return FALSE;
  }

  if (!pBuf) {
    ShowError(MB_OK|MB_ICONSTOP, ERROR_INVALID_NAME, _T("GetFullPathName(%s)"),pNearPath);
    return FALSE;
  }
  ...
  //ShowMsg(MB_OK, _T("pFilePath=%s\n"),pFilePath);

  return TRUE;
}

InfFile::InfFile(const TCHAR *pInfName, const TCHAR *pNearPath)
  : pPath(NULL)
  , pOemPath(NULL)
  , pClassGUID(NULL)
  , pClass(NULL)
...
bool InfFile::Test(const InfFileField *pFields, bool showErrors) const
{
  ...
  for (const InfFileField *pField = pFields ; pField->pSection != NULL ; pField++) {
    ...
    for (;;) {
      ...
      TCHAR *pValue = (TCHAR *)LocalAlloc(LPTR, size*sizeof(pValue[0]));

      if (pValue == NULL) {
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        ShowLastError(MB_OK|MB_ICONSTOP, _T("LocalAlloc(%lu)"),(unsigned long)(size*sizeof(pValue[0])));
        return FALSE;
      }
...
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::OemPath(bool showErrors) const
{
  if (!pOemPath) {
    TCHAR path[MAX_PATH + 1];
...
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::ClassGUID(bool showErrors) const
{
  GetVersionInfo(pPath, _T("ClassGUID"),&pClassGUID, showErrors);

  return pClassGUID;
}
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::Class(bool showErrors) const
{
  GetVersionInfo(pPath, _T("Class"),&pClass, showErrors);

  return pClass;
}
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::Provider(bool showErrors) const
{
  GetVersionInfo(pPath, _T("Provider"),&pProvider, showErrors);

  return pProvider;
}
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::DriverVer(bool showErrors) const
{
  GetVersionInfo(pPath, _T("DriverVer"),&pDriverVer, showErrors);

  return pDriverVer;
}
///////////////////////////////////////////////////////////////
const TCHAR *InfFile::UninstallInfTag(bool showErrors) const
{
  GetVersionInfo(pPath, _T("UninstallInfTag"),&pUninstallInfTag, showErrors);
    ...
    UINT_PTR Param2)
{
  if (Notification == SPFILENOTIFY_ENDDELETE) {
    ...
    if (pFilePaths->Win32Error == ERROR_SUCCESS) {
      Trace(_T("Deleted File %s\n"),pFilePaths->Target);
    }
    else
    if (pFilePaths->Win32Error == ERROR_FILE_NOT_FOUND) {
      Trace(_T("File %s not installed\n"),pFilePaths->Target);
    }
    else {
      ShowError(MB_OK|MB_ICONWARNING, pFilePaths->Win32Error, _T("Delete(%s)"),pFilePaths->Target);
    }
  }
...
bool InfFile::UninstallFiles(const TCHAR *pFilesSection) const
{
  if (!pPath)
    return FALSE;
      ...
      if (SetupQueueDeleteSection(hFileQueue, hInf, NULL, pFilesSection)) {
        ...
        if (pContext) {
          if(!SetupCommitFileQueue(NULL, hFileQueue, FileCallback, pContext))
            res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupCommitFileQueue()"));

          SetupTermDefaultQueueCallback(pContext);
        } else {
          res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupInitDefaultQueueCallback()"));
        }
      } else {
        res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupQueueDeleteSection(%s)"),pFilesSection);
      }

      SetupCloseFileQueue(hFileQueue);
    } else {
      res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupOpenFileQueue()"));
    }
...
bool InfFile::InstallOEMInf() const
{
  ...
  if (!SetupCopyOEMInf(pPath, NULL, SPOST_PATH, 0, NULL, 0, NULL, NULL)) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupCopyOEMInf(%s)"),pPath);
    return FALSE;
  }

  if (!wasInstalled)
    Trace(_T("Installed %s to %s\n"),pPath, OemPath());
  else
    Trace(_T("Re-installed %s to %s\n"),pPath, OemPath());

  return TRUE;
}
///////////////////////////////////////////////////////////////
static bool UninstallFile(const TCHAR *pPath)
{
  int res;
  ...
  do {
    ...
    if (GetLastError() == ERROR_FILE_NOT_FOUND) {
      ...
      res = ShowLastError(MB_CANCELTRYCONTINUE, _T("DeleteFile(%s)"),pPath);
    }
  } while (res == IDTRYAGAIN);
...
static bool UninstallInf(const TCHAR *pPath)
{
  if (pPath == NULL)
    return FALSE;
  ...
  do {
    ...
    TCHAR infPathDest[MAX_PATH + 1];

    if (SNPRINTF(infPathDest, sizeof(infPathDest)/sizeof(infPathDest[0]), _T("%s"),pPath) > 0) {
        typedef BOOL(WINAPI* PSETUPUNINSTALLOEMINFA)(IN PCSTR, IN DWORD, IN PVOID);
        typedef BOOL(WINAPI* PSETUPUNINSTALLOEMINFW)(IN PCWSTR, IN DWORD, IN PVOID);
#ifdef _UNICODE
      static PSETUPUNINSTALLOEMINFW pSetupUninstallOEMInf = NULL;
#else
      static PSETUPUNINSTALLOEMINFA pSetupUninstallOEMInf = NULL;
#endif
      if(!pSetupUninstallOEMInf) {
        HMODULE hModule = GetModuleHandle(_T("setupapi.dll"));

        if (hModule) {
#ifdef _UNICODE
            pSetupUninstallOEMInf =
                (PSETUPUNINSTALLOEMINFW)GetProcAddress(hModule, "SetupUninstallOEMInfW");
#else
          pSetupUninstallOEMInf =
      ...
      if (pSetupUninstallOEMInf) {
        ...
        if (pSetupUninstallOEMInf(pInfNameDest, 0, NULL)) {
          Trace(_T("Uninstalled %s\n"),pInfNameDest);
        } else {
          res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupUninstallOEMInf(%s)"),pInfNameDest);
        }
      }
      else
      if (UninstallFile(infPathDest)) {
        ...
        if (infPathDestLen > 4) {
          TCHAR *pInfPathDestExt = infPathDest + infPathDestLen - 4;

          if (!lstrcmpi(pInfPathDestExt, _T(".inf"))) {
  ...
  do {
    ...
    if (OemPath()) {
      ...
      if (GetLastError() == ERROR_FILE_NOT_FOUND) {
        ...
        res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupCopyOEMInf(%s)"),pPath);
      }
    }
  } while (res == IDTRYAGAIN);
    ...
    const TCHAR *const *ppOemPathExcludeList)
{
  ...
  if (!SetupGetInfFileList(NULL, INF_STYLE_WIN4, NULL, 0, &size)) {
    ...
    Trace(_T(" FAIL\n"));
    ShowError(MB_OK|MB_ICONSTOP, err, _T("SetupGetInfFileList()"));
    return FALSE;
  }
  ...
  TCHAR *pList = (TCHAR *)LocalAlloc(LPTR, size*sizeof(pList[0]));
  ...
  if (pList) {
    if (!SetupGetInfFileList(NULL, INF_STYLE_WIN4, pList, size, NULL)) {
      ...
      Trace(_T(" FAIL\n"));
      ShowError(MB_OK|MB_ICONSTOP, err, _T("SetupGetInfFileList(%lu)"),(unsigned long)size);
      LocalFree(pList);
      return FALSE;
    }
  } else {
  ...
  size = GetEnvironmentVariable(_T("windir"),windir, sizeof(windir)/sizeof(windir[0]));

  if (!size || size >= sizeof(windir)/sizeof(windir[0])) {
    DWORD err = !size ? GetLastError() : ERROR_BUFFER_OVERFLOW;

    Trace(_T(" FAIL\n"));
    ShowError(MB_OK|MB_ICONSTOP, err, _T("GetEnvironmentVariable(windir)"));
    LocalFree(pList);
    return FALSE;
  }
  ...
  do {
    ...
    TCHAR infPath[MAX_PATH + 1];

    if ((p[0] == 'O' || p[0] == 'o') &&
        (p[1] == 'E' || p[1] == 'e') &&
        (p[2] == 'M' || p[2] == 'm') &&
        SNPRINTF(infPath, sizeof(infPath)/sizeof(infPath[0]), _T("%s\\inf\\%s"),windir, p) > 0)
    {
      ...
      if (TestUninstall(infFile, pInfFileUninstallList, TRUE)) {
        ...
        res = ShowMsg(MB_YESNO,
            _T("The file %s possible should be deleted too.\n"
            "\n"
            "%s:\n"
            "  ClassGUID = %s\n"
            "  Class = %s\n"
