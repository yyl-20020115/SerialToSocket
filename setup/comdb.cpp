...
#include <tchar.h>
///////////////////////////////////////////////////////////////
static const TCHAR comDbLocalKey[] = REGSTR_PATH_SERVICES _T("\\") C0C_SERVICE _T("\\COM Name Arbiter");
static const TCHAR comDbLocalName[] = _T("ComDB");
///////////////////////////////////////////////////////////////
static WORD name2num(const TCHAR*pPortName)
{
  int num;

  if ((pPortName[0] != _T('C') && pPortName[0] != 'c') ||
...
static bool LoadComDb(BusyMask &comDb)
{
  ...
  do {
    ...
    if (err != ERROR_SUCCESS) {
      res = ShowLastError(MB_CANCELTRYCONTINUE, _T("ComDBOpen()"));
      continue;
    }
    ...
    if (err != ERROR_SUCCESS) {
      ComDBClose(hComDB);
      res = ShowError(MB_CANCELTRYCONTINUE, err, _T("ComDBGetCurrentPortUsage()"));
      continue;
    }
    ...
    if (!pBuf) {
      ...
      res = ShowError(MB_CANCELTRYCONTINUE, ERROR_NOT_ENOUGH_MEMORY, _T("LocalAlloc(%lu)"), (unsigned long)bufSize);
      continue;
    }
...
static bool ClaimReleasePort(DWORD num, bool claim)
{
  ...
  do {
    ...
    if (err != ERROR_SUCCESS) {
      res = ShowLastError(MB_RETRYCANCEL, _T("ComDBOpen()"));
      continue;
    }
    ...
    if (claim) {
      ...
      if (err != ERROR_SUCCESS) {
        ...
        if (err == ERROR_SHARING_VIOLATION)
          ...
          res = ShowError(MB_RETRYCANCEL, err, _T("ComDBClaimPort(COM%u)"), (unsigned)num);

        continue;
      } else {
        Trace(_T("ComDB: COM%u - logged as \"in use\"\n"), unsigned(num));
      }
    } else {
      ...
      if (err != ERROR_SUCCESS) {
        ...
        res = ShowError(MB_RETRYCANCEL, err, _T("ComDBReleasePort(COM%u)"), (unsigned)num);
        continue;
      } else {
        Trace(_T("ComDB: COM%u - released\n"), unsigned(num));
      }
    }
  ...
  do {
    ...
    if (err != ERROR_SUCCESS) {
      if (err != ERROR_FILE_NOT_FOUND)
        res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegOpenKeyEx(%s)"), comDbLocalKey);
    ...
    if (err != ERROR_SUCCESS) {
      ...
      if (err != ERROR_FILE_NOT_FOUND)
        res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegQueryValueEx(%s\\%s)"), comDbLocalKey, comDbLocalName);
...
static bool SaveComDbLocal(const BusyMask &comDb)
{
  ...
  do {
    ...
    if (err != ERROR_SUCCESS) {
      res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegCreateKeyEx(%s)"), comDbLocalKey);
      continue;
    }
    ...
    if (!maxPortsReported) {
      ...
      if (err != ERROR_SUCCESS && err != ERROR_FILE_NOT_FOUND)
        res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegDeleteValue(%s\\%s)"), comDbLocalKey, comDbLocalName);
    ...
    if (err != ERROR_SUCCESS) {
      res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegSetValueEx(%s\\%s)"), comDbLocalKey, comDbLocalName);
      continue;
    }
  } while (res == IDTRYAGAIN);
...
///////////////////////////////////////////////////////////////
bool ComDbGetInUse(const TCHAR *pPortName, bool &inUse)
{
...
///////////////////////////////////////////////////////////////
DWORD ComDbQueryNames(TCHAR *pBuf, DWORD maxChars)
{
  BusyMask comDb;
  ...
  for (DWORD num = 0 ; num < COMDB_MAX_PORTS_ARBITRATED ; num++) {
    ...
    int len = SNPRINTF(pB, maxChars, _T("COM%u"), (unsigned)(num + 1));
...
///////////////////////////////////////////////////////////////
bool ComDbClaim(const TCHAR *pPortName)
{
  return ClaimReleasePort(name2num(pPortName), TRUE);
}
///////////////////////////////////////////////////////////////
bool ComDbRelease(const TCHAR *pPortName)
{
  return ClaimReleasePort(name2num(pPortName), FALSE);
}
///////////////////////////////////////////////////////////////
bool ComDbIsValidName(const TCHAR *pPortName)
{
  if (name2num(pPortName) == 0) {
    ShowMsg(MB_OK|MB_ICONSTOP, _T("The port name %s is not in COM<1-%u> format.\n"),
