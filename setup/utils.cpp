...
///////////////////////////////////////////////////////////////
int VSNPRINTF(TCHAR*pBuf, int size, const TCHAR*pFmt, va_list va)
{
    TCHAR buf[1025];
...
///////////////////////////////////////////////////////////////
int SNPRINTF(TCHAR*pBuf, int size, const TCHAR*pFmt, ...)
{
  va_list va;
...
///////////////////////////////////////////////////////////////
static bool IsDelim(TCHAR c, const TCHAR*pDelims)
{
  while (*pDelims) {
    if (c == *pDelims++)
      return TRUE;
...
TCHAR *STRTOK_R(TCHAR*pStr, const TCHAR*pDelims, TCHAR**ppSave)
{
  if (!pStr)
    pStr = *ppSave;
...
///////////////////////////////////////////////////////////////
bool StrToInt(const TCHAR*pStr, int *pNum)
{
  bool res = FALSE;
  int num;
  int sign = 1;
...
///////////////////////////////////////////////////////////////
bool MatchPattern(const TCHAR*pPattern, const TCHAR*pStr)
{
  for (;;) {
    switch (*pPattern) {
      case '*':
...
///////////////////////////////////////////////////////////////
TCHAR *STRDUP(const TCHAR*pSrcStr, bool showErrors)
{
  TCHAR *pDstStr;
  int len = lstrlen(pSrcStr) + 1;

  pDstStr = (TCHAR *)LocalAlloc(LPTR, len*sizeof(pDstStr[0]));

  if (pDstStr != NULL) {
    SNPRINTF(pDstStr, len, _T("%s"), pSrcStr);
  } else {
    SetLastError(ERROR_NOT_ENOUGH_MEMORY);
    if (showErrors)
      ShowLastError(MB_OK|MB_ICONSTOP, _T("LocalAlloc(%lu)"), (unsigned long)(len*sizeof(pDstStr[0])));
  }
