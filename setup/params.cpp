...
///////////////////////////////////////////////////////////////
PortParameters::PortParameters(const TCHAR *pService, const TCHAR *pPhPortName)
{
  SNPRINTF(service, sizeof(service)/sizeof(service[0]), _T("%s"), pService);
  SNPRINTF(phPortName, sizeof(phPortName)/sizeof(phPortName[0]), _T("%s"), pPhPortName);
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetPortName(const TCHAR *pNewPortName)
{
  if (lstrcmpi(portName, pNewPortName) != 0) {
    if (SNPRINTF(portName, sizeof(portName)/sizeof(portName[0]), _T("%s"), pNewPortName) <= 0) {
      Trace(_T("Invalid port name '%s'\n"), pNewPortName);
      return FALSE;
    }
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetRealPortName(const TCHAR *pNewRealPortName)
{
  if (lstrcmpi(realPortName, pNewRealPortName) != 0) {
    if (SNPRINTF(realPortName, sizeof(realPortName)/sizeof(realPortName[0]), _T("%s"), pNewRealPortName) <= 0) {
      Trace(_T("Invalid real port name '%s'\n"), pNewRealPortName);
      return FALSE;
    }
...
///////////////////////////////////////////////////////////////
bool PortParameters::InitRealPortName(const TCHAR *pRealPortName)
{
  if (!lstrcmpi(_T(""), pRealPortName)) {
  ...
  if (SNPRINTF(realPortName, sizeof(realPortName)/sizeof(realPortName[0]), _T("%s"), pRealPortName) <= 0) {
    Trace(_T("Invalid real port name '%s'\n"), pRealPortName);
    return FALSE;
  }
...
///////////////////////////////////////////////////////////////
static const TCHAR *GetBitName(DWORD bit)
{
  switch (bit) {
    case m_portName:       return _T("PortName");
    case m_realPortName:   return _T("RealPortName");
    case m_emuBR:          return _T("EmuBR");
    case m_emuOverrun:     return _T("EmuOverrun");
    case m_plugInMode:     return _T("PlugInMode");
    case m_exclusiveMode:  return _T("ExclusiveMode");
    case m_hiddenMode:     return _T("HiddenMode");
    case m_allDataBits:    return _T("AllDataBits");
    case m_pinCTS:         return _T("cts");
    case m_pinDSR:         return _T("dsr");
    case m_pinDCD:         return _T("dcd");
    case m_pinRI:          return _T("ri");
    case m_emuNoise:       return _T("EmuNoise");
    case m_addRTTO:        return _T("AddRTTO");
    case m_addRITO:        return _T("AddRITO");
  }

  return NULL;
}
///////////////////////////////////////////////////////////////
bool PortParameters::SetFlag(const TCHAR *pNewVal, DWORD bit)
{
  DWORD newFlag;

  if (!lstrcmpi(pNewVal, _T("yes"))) {
    newFlag = 0xFFFFFFFF;
  }
  else
  if (!lstrcmpi(pNewVal, _T("no"))) {
    newFlag = 0;
  }
  else {
    Trace(_T("Invalid value '%s'\n"), pNewVal);
    return FALSE;
  }
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetPin(const TCHAR *pNewVal, DWORD bit)
{
  DWORD newPin;
  ...
  if (!lstrcmpi(pNewVal, _T("rrts"))) {
    newPin |= C0C_PIN_RRTS;
  }
  else
  if (!lstrcmpi(pNewVal, _T("rdtr"))) {
    newPin |= C0C_PIN_RDTR;
  }
  else
  if (!lstrcmpi(pNewVal, _T("rout1"))) {
    newPin |= C0C_PIN_ROUT1;
  }
  else
  if (!lstrcmpi(pNewVal, _T("rout2"))) {
    newPin |= C0C_PIN_ROUT2;
  }
  else
  if (!lstrcmpi(pNewVal, _T("ropen"))) {
    newPin |= C0C_PIN_ROPEN;
  }
  else
  if (!lstrcmpi(pNewVal, _T("lrts"))) {
    newPin |= C0C_PIN_LRTS;
  }
  else
  if (!lstrcmpi(pNewVal, _T("ldtr"))) {
    newPin |= C0C_PIN_LDTR;
  }
  else
  if (!lstrcmpi(pNewVal, _T("lout1"))) {
    newPin |= C0C_PIN_LOUT1;
  }
  else
  if (!lstrcmpi(pNewVal, _T("lout2"))) {
    newPin |= C0C_PIN_LOUT2;
  }
  else
  if (!lstrcmpi(pNewVal, _T("lopen"))) {
    newPin |= C0C_PIN_LOPEN;
  }
  else
  if (!lstrcmpi(pNewVal, _T("on"))) {
    newPin |= C0C_PIN_ON;
  }
  else {
    Trace(_T("Invalid value '%s'\n"), pNewVal);
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetProbability(const TCHAR *pNewVal, DWORD bit)
{
  DWORD newVal = 0;

  if (pNewVal[0] == '0' && pNewVal[1] == '.') {
    const TCHAR *p = pNewVal + 2;
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetUnsigned(const TCHAR *pNewVal, DWORD bit)
{
  DWORD newVal = 0;

  for (const TCHAR *p = pNewVal ; *p ; p++) {
    if (*p < '0' || *p > '9') {
      Trace(_T("Invalid value '%s'\n"), pNewVal);
      return FALSE;
    }
...
///////////////////////////////////////////////////////////////
bool PortParameters::SetBit(const TCHAR *pVal, const Bit &bit)
{
  if (!lstrcmpi(pVal, _T("*")))
    return TRUE;
...
///////////////////////////////////////////////////////////////
bool PortParameters::FillParametersKey(TCHAR *pRegKey, int size)
{
  int len;
...
void PortParameters::LoadDw(HKEY hKey, DWORD bit)
{
  ...
  const TCHAR *pName = GetBitName(bit);
...
LONG PortParameters::Load()
{
  ...
  if (err == ERROR_SUCCESS) {
    SNPRINTF(portName, sizeof(portName)/sizeof(portName[0]), _T("%s"), (TCHAR *)buf);
...
LONG PortParameters::Save()
{
  ...
  if (maskChanged & m_portName) {
    if (maskExplicit & m_portName) {
      ...
      err = RegDeleteValue(hKey, _T("PortName"));
...
///////////////////////////////////////////////////////////////
bool PortParameters::ParseParametersStr(const TCHAR *pParameters)
{
  PortParameters tmp = *this;

  if (!lstrcmpi(pParameters, _T("-")) || !lstrcmpi(pParameters, _T("*"))) {
    int i;
    ...
    if (SNPRINTF(pars, sizeof(pars)/sizeof(pars[0]), _T("%s"), pParameters) < 0) {
      Trace(_T("The parameters string '%s' is too long\n"), pParameters);
      return FALSE;
    }

    TCHAR *pSave1;

    for (TCHAR *pPar = STRTOK_R(pars, _T(","), &pSave1) ; pPar ; pPar = STRTOK_R(NULL, _T(","), &pSave1)) {
      TCHAR *pSave2;
      const TCHAR *pKey = STRTOK_R(pPar, _T("="), &pSave2);
      const TCHAR *pVal = STRTOK_R(NULL, _T("="), &pSave2);

      //Trace(_T("'%s'='%s'\n", pKey, pVal);

      if (!pVal) {
        Trace(_T("Missing value for '%s'\n"), pKey);
        return FALSE;
      }
...
///////////////////////////////////////////////////////////////
bool PortParameters::FillParametersStr(TCHAR *pParameters, int size, bool detail)
{
  int len;

  len = SNPRINTF(pParameters, size, _T("PortName=%s")),
                 (maskExplicit & m_portName) ? portName : (detail ? phPortName : _T("-"));
  ...
  if (maskExplicit & m_realPortName) {
    len = SNPRINTF(pParameters, size, _T(",RealPortName=%s"), realPortName);
  ...
  for (int i = 0 ; i < sizeof(bits)/sizeof(bits[0]) ; i++) {
    ...
    if (pName) {
      ...
      if (bits[i].type == Bit::FLAG) {
        len = SNPRINTF(pParameters, size, _T(",%s=%s"), pName, *pBit ? "yes" : "no");
      }
      else
      if (bits[i].type == Bit::PIN) {
        ...
        switch (*pBit & ~C0C_PIN_NEGATIVE) {
          case C0C_PIN_RRTS:  pVal = _T("rrts");  break;
          case C0C_PIN_RDTR:  pVal = _T("rdtr");  break;
          case C0C_PIN_ROUT1: pVal = _T("rout1"); break;
          case C0C_PIN_ROUT2: pVal = _T("rout2"); break;
          case C0C_PIN_ROPEN: pVal = _T("ropen"); break;
          case C0C_PIN_LRTS:  pVal = _T("lrts");  break;
          case C0C_PIN_LDTR:  pVal = _T("ldtr");  break;
          case C0C_PIN_LOUT1: pVal = _T("lout1"); break;
          case C0C_PIN_LOUT2: pVal = _T("lout2"); break;
          case C0C_PIN_LOPEN: pVal = _T("lopen"); break;
          case C0C_PIN_ON:    pVal = _T("on");    break;
        }

        if (pVal == NULL)
          continue;

        len = SNPRINTF(pParameters, size, _T(",%s=%s%s"), pName, (*pBit & C0C_PIN_NEGATIVE) == 0 ? "" : "!", pVal);
      }
      else
      if (bits[i].type == Bit::PROBABILITY) {
          if (*pBit == 0) {
            len = SNPRINTF(pParameters, size, _T(",%s=0"), pName);
          } else {
            TCHAR strVal[11] = _T("");
            TCHAR digits[11];

            SNPRINTF(digits, sizeof(digits)/sizeof(digits[0]), _T("%ld"), (unsigned long)*pBit);
            ...
            len = SNPRINTF(pParameters, size, _T(",%s=0.%s"), pName, strVal);
          }
      }
      else
      if (bits[i].type == Bit::UNSIGNED) {
        len = SNPRINTF(pParameters, size, _T(",%s=%lu"), pName, (unsigned long)*pBit);
      }
...
///////////////////////////////////////////////////////////////
bool PortParameters::FillPortName(TCHAR *pPortName, int size)
{
  int len;

  len = SNPRINTF(pPortName, size, _T("%s"),
                 (maskExplicit & m_portName) ? portName : phPortName);

  return len >= 0;
}
///////////////////////////////////////////////////////////////
bool PortParameters::FillRealPortName(TCHAR *pRealPortName, int size)
{
  int len;

  len = SNPRINTF(pRealPortName, size, _T("%s"),
                 (maskExplicit & m_realPortName) ? realPortName : _T(""));

  return len >= 0;
}
///////////////////////////////////////////////////////////////
const TCHAR *PortParameters::GetHelp()
{
  return
    _T("Syntax of port parameters string:\n"
