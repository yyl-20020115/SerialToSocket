...
#define C0C_INF_NAME             _T("com0com.inf")
#define C0C_INF_NAME_CNCPORT     _T("cncport.inf")
#define C0C_INF_NAME_COMPORT     _T("comport.inf")
#define C0C_CNCCLASS_GUID        _T("{df799e12-3c56-421b-b298-b6d3642bc878}")
#define C0C_COMCLASS_GUID        _T("{4d36e978-e325-11ce-bfc1-08002be10318}")
#define C0C_PROVIDER             _T("Vyacheslav Frolov")
#define C0C_REGKEY_EVENTLOG      REGSTR_PATH_SERVICES _T("\\Eventlog\\System\\") C0C_SERVICE
#define C0C_COPY_DRIVERS_SECTION _T("com0com_CopyDrivers")

#define C0C_SETUP_TITLE          _T("Setup for com0com")
///////////////////////////////////////////////////////////////
static const InfFile::InfFileField requiredFieldsInfBusInstall[] = {
  { _T("Version"),         _T("ClassGUID"),          1, C0C_CNCCLASS_GUID },
  { _T("Version"),         _T("Provider"),           1, C0C_PROVIDER },
  { _T("Version"),         _T("UninstallInfTag"),    1, C0C_CNCCLASS_GUID },
  { _T("Manufacturer"),    NULL,                 1, C0C_SERVICE },
  { NULL },
};
...
static const InfFile::InfFileField requiredFieldsInfCOMPortInstall[] = {
  { _T("Version"),         _T("ClassGUID"),          1, C0C_COMCLASS_GUID },
  { _T("Version"),         _T("Provider"),           1, C0C_PROVIDER },
  { _T("Version"),         _T("UninstallInfTag"),    1, C0C_CNCCLASS_GUID },
  { _T("Manufacturer"),    NULL,                 1, C0C_SERVICE },
  { NULL },
...
struct InfFileInstall {
  const TCHAR *pInfName;
  const TCHAR *pCopyDriversSection;
  const TCHAR *pHardwareId;
...
static const InfFile::InfFileField requiredFieldsInfUnnstallCNCOld[] = {
  { _T("Version"),         _T("ClassGUID"),          1, C0C_CNCCLASS_GUID },
  { _T("Version"),         _T("Provider"),           1, C0C_PROVIDER },
  { _T("Version"),         _T("CatalogFile"),        1, _T("com0com.cat") },
  { NULL },
};
...
///////////////////////////////////////////////////////////////
bool IsComClass(const TCHAR *pPortName)
{
  return lstrcmpi(pPortName, C0C_PORT_NAME_COMCLASS) == 0;
}
///////////////////////////////////////////////////////////////
static CNC_ENUM_FILTER EnumFilter;
static bool EnumFilter(const TCHAR *pHardwareId)
{
  if (!pHardwareId)
    return FALSE;
...
static bool FillPortDevProperties(DevProperties &devProperties, int iBus, bool isA)
{
  ...
  SNPRINTF(phDevName, sizeof(phDevName)/sizeof(phDevName[0]), _T("%s%d"),
...
static bool IsValidPortNum(int num)
{
  ...
  TCHAR buf[C0C_PORT_NAME_LEN + 1];

  if (SNPRINTF(buf, sizeof(buf)/sizeof(buf[0]), C0C_PREF_BUS_NAME _T("%d"), num) < 0 ||
      SNPRINTF(buf, sizeof(buf)/sizeof(buf[0]), C0C_PREF_PORT_NAME_A _T("%d"), num) < 0 ||
      SNPRINTF(buf, sizeof(buf)/sizeof(buf[0]), C0C_PREF_PORT_NAME_B _T("%d"), num) < 0)
  {
    int res = ShowMsg(MB_OKCANCEL|MB_ICONWARNING, _T("The port number %d is too big.\n"), num);
    ...
    const TCHAR *pPortName)
{
  ...
  if (len <= 0) {
    ShowMsg(MB_OK|MB_ICONSTOP, _T("The port name is empty.\n"));
    return FALSE;
  }
  else
  if (len > C0C_PORT_NAME_LEN) {
    res = ShowMsg(MB_OKCANCEL|MB_ICONWARNING,
                  _T("The length of port name %s\n"
  ...
  do {
    ...
    res = ShowMsg(MB_CANCELTRYCONTINUE,
                  _T("The port name %s is already used for other device %s.\n"),
                  pPortName, phDevName);
  ...
  do {
    ...
    if (inUse) {
      res = ShowMsg(MB_CANCELTRYCONTINUE,
                    _T("The port name %s is already logged as \"in use\"\n"
    ...
    int num)
{
  ...
  for (int j = 0 ; j < 2 ; j++) {
    ...
    SNPRINTF(phPortName, sizeof(phPortName)/sizeof(phPortName[0]), _T("%s%d"),
    ...
    if (portParameters.Load() == ERROR_SUCCESS)
      ...
      SNPRINTF(portName[j], sizeof(portName[j])/sizeof(portName[j][0]), _T("%s"), phPortName);
  }

  TCHAR friendlyNameOld[120];
  ...
  {
    SNPRINTF(friendlyNameOld, sizeof(friendlyNameOld)/sizeof(friendlyNameOld[0]), _T(""));
  }

  if (show_fnames)
    Trace(_T("       ") C0C_PREF_BUS_NAME _T("%d FriendlyName=\"%s\"\n"), num, friendlyNameOld);
  ...
  if (!no_update_fnames) {
                                          ...
                                          (LPBYTE)deviceDesc, sizeof(deviceDesc), NULL))
    {
      SNPRINTF(deviceDesc, sizeof(deviceDesc)/sizeof(deviceDesc[0]),
              _T( "com0com - bus for serial port pair emulator"));
    }
    ...
    if (lstrcmp(friendlyName, friendlyNameOld) != 0) {
      ...
      {
        if (show_fnames)
          Trace(_T("update ") C0C_PREF_BUS_NAME _T("%d FriendlyName=\"%s\"\n"), num, friendlyName);
      }
    }
  }
}
...
static VOID SetFriendlyNamePort(
    HDEVINFO hDevInfo,
    PSP_DEVINFO_DATA pDevInfoData,
    const TCHAR *pPhObjName)
{
  //Trace(_T("SetFriendlyNamePort pPhObjName=%s\n"), pPhObjName);
  ...
  for (int j = 0 ;; j++) {
    ...
    const TCHAR *pPref = (j ? C0C_PREF_DEVICE_NAME_B : C0C_PREF_DEVICE_NAME_A);
  ...
  //Trace(_T("SetFriendlyNamePort phPortName=%s\n"), phPortName);
  ...
  if (portParameters.Load() == ERROR_SUCCESS)
    ...
    SNPRINTF(portName, sizeof(portName)/sizeof(portName[0]), _T("%s"), phPortName);

  //Trace(_T("SetFriendlyNamePort portName=%s\n"), portName);
  ...
  if (IsComClass(portName)) {
    ...
    LONG err = RegQueryValueEx(hKey, _T("PortName"), NULL, NULL, (PBYTE)portName, &len);
  ...
  if (!no_update_fnames) {
    ...
    {
      SNPRINTF(deviceDesc, sizeof(deviceDesc)/sizeof(deviceDesc[0]),
          _T("com0com - serial port emulator"));
    }

    TCHAR friendlyName[120];

    SNPRINTF(friendlyName, sizeof(friendlyName)/sizeof(friendlyName[0]), _T("%s %s (%s)"), deviceDesc, phPortName, portName);

    //Trace(_T("SetFriendlyNamePort friendlyName=%s\n"), friendlyName);
    ...
    if (lstrcmp(friendlyName, friendlyNameOld) != 0) {
      ...
      {
        if (show_fnames)
          Trace(_T("update %s FriendlyName=\"%s\"\n"), phPortName, friendlyName);
      }
    }
  }
}
    ...
    void *pParam)
{
  if (!lstrcmp(pDevProperties->DevId(), C0C_PORT_DEVICE_ID)) {
    ...
    if (hKey != INVALID_HANDLE_VALUE) {
      TCHAR portNameOld[20];
      DWORD len = sizeof(portNameOld);

      LONG err = RegQueryValueEx(hKey, _T("PortName"), NULL, NULL, (PBYTE)portNameOld, &len);
    ...
    void *pParam)
{
  if (!lstrcmp(pDevProperties->DevId(), C0C_PORT_DEVICE_ID)) {
    ...
    if (hKey != INVALID_HANDLE_VALUE) {
      ...
      if (err == ERROR_SUCCESS) {
        const TCHAR*pPortNameNew = (const TCHAR*)pParam;

        if (ComDbClaim(pPortNameNew)) {
          err = RegSetValueEx(hKey, _T("PortName"), 0, REG_SZ,
                              (PBYTE)pPortNameNew, (lstrlen(pPortNameNew) + 1) * sizeof(pPortNameNew[0]));

          if (err == ERROR_SUCCESS) {
            ComDbRelease((const TCHAR*)portNameOld);
            SetFriendlyNamePort(hDevInfo, pDevInfoData, pDevProperties->PhObjName());
          } else {
...
struct ChangeDeviceParams {
  ChangeDeviceParams(
    const TCHAR *_pPhPortName,
    const TCHAR *_pParameters)
  : pPhPortName(_pPhPortName), pParameters(_pParameters), changed(FALSE) {}

  const TCHAR *pPhPortName;
  const TCHAR *pParameters;
  bool changed;
};
    ...
    void *pParam)
{
  ...
  if (i >= 0) {
    const TCHAR *pPhPortName = ((ChangeDeviceParams *)pParam)->pPhPortName;
    const TCHAR *pParameters = ((ChangeDeviceParams *)pParam)->pParameters;
    ...
    for (int j = 0 ; j < 2 ; j++) {
      ...
      if (err != ERROR_SUCCESS) {
        ...
        Trace(_T("       %s %s\n"), phPortName, buf);
        ShowError(MB_OK|MB_ICONWARNING, err, _T("portParameters.Load(%s)"), phPortName);
        continue;
      }
      ...
      if (err != ERROR_SUCCESS) {
        ShowError(MB_OK|MB_ICONWARNING, err, _T("portParameters.Save(%s)"), phPortName);
        continue;
      }
      ...
      if (isComClassNew != isComClassOld) {
        Trace(_T("Changed port class for %s (renamed %s to %s)\n"), phPortName, portNameOld, portNameNew);
    ...
    Trace(_T("       " C0C_PREF_PORT_NAME_A "?\n"));
    Trace(_T("       " C0C_PREF_PORT_NAME_B "?\n"));
  }

  return TRUE;
}

bool Change(const TCHAR *pPhPortName, const TCHAR *pParameters)
{
    ...
    if (pInfFileInstall->preinstallClass) {
      ...
      do {
        ...
        if (SetupDiInstallClass(NULL, infFile.Path(), Silent() ? 0 : DI_QUIETINSTALL, NULL)) {
          Trace(_T("Installed Class %s\n"), infFile.ClassGUID());
        } else {
          res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupDiInstallClass()"));
        }
      } while (res == IDTRYAGAIN);
...
///////////////////////////////////////////////////////////////
bool Install(const TCHAR *pInfFilePath)
{
  if (no_update)
    return TRUE;
    ...
    void *pParam)
{
  if (!lstrcmp(pDevProperties->DevId(), C0C_BUS_DEVICE_ID)) {
    ...
    do {
      ...
      if (err != ERROR_SUCCESS)
        res = ShowError(MB_CANCELTRYCONTINUE, err, _T("SetPortNum(%d)"), num);
...
static bool InstallBusDevice(const TCHAR *pInfFilePath, int num)
{
  BOOL rebootRequired = FALSE;
    ...
    void *pParam)
{
  ...
  if (i >= 0) {
    if (!((BusyMask *)pParam)->AddNum(i)) {
      if (ShowLastError(MB_OKCANCEL|MB_ICONWARNING, _T("AddDeviceToBusyMask(%d)"), i) != IDOK)
        return FALSE;
    }
  }
...
bool Install(const TCHAR *pInfFilePath, const TCHAR *pParametersA, const TCHAR *pParametersB, int num)
{
  int i;
  int res;
  ...
  if (lstrcmpi(portName[0], portName[1]) == 0 && !IsComClass(portName[0]) &&
      ShowMsg(MB_OKCANCEL|MB_ICONWARNING,
          _T("The same port name %s is used for both ports.\n"),
              portName[0]) != IDOK)
  {
    goto err;
  }
    ...
    const InfFile::InfFileUninstall *pInfFileUninstallList)
{
        ...
        if (hSrv) {
          ...
          if (QueryServiceStatus(hSrv, &srvStatus)) {
            if (srvStatus.dwCurrentState == SERVICE_STOPPED) {
              if (Silent() ||
                  ShowMsg(MB_YESNO,
                      _T("The deleting %s service will remove your manual settings.\n"
                  "Would you like to delete service?\n"),
                  C0C_SERVICE) == IDYES)
              {
                if (DeleteService(hSrv)) {
                  Trace(_T("Deleted Service %s\n"), C0C_SERVICE);
                  notDeleted = FALSE;
                } else {
                  res = ShowLastError(MB_CANCELTRYCONTINUE, _T("DeleteService(%s)"), C0C_SERVICE);
                }
              }
            } else {
              res = ShowMsg(MB_CANCELTRYCONTINUE,
                  _T("Service %s is not stopped (state %ld).\n"),
                            C0C_SERVICE, (long)srvStatus.dwCurrentState);
            }
          } else {
            res = ShowLastError(MB_CANCELTRYCONTINUE, _T("QueryServiceStatus(%s)"), C0C_SERVICE);
          }

          if (!CloseServiceHandle(hSrv))
            ShowLastError(MB_OK|MB_ICONWARNING, _T("CloseServiceHandle(hSrv)"));
        } else {
          if (GetLastError() == ERROR_SERVICE_DOES_NOT_EXIST) {
            Trace(_T("Service %s not installed\n"), C0C_SERVICE);
            notDeleted = FALSE;
          } else {
            res = ShowLastError(MB_CANCELTRYCONTINUE, _T("OpenService(%s)"), C0C_SERVICE);
          }
        }
      } while (res == IDTRYAGAIN);

      if (!CloseServiceHandle(hSCM))
        ShowLastError(MB_OK|MB_ICONWARNING, _T("CloseServiceHandle(hSCM)"));
    } else {
      res = ShowLastError(MB_CANCELTRYCONTINUE, _T("OpenSCManager()"));
    }
  } while (res == IDTRYAGAIN);

  if (notDeleted)
    Trace(_T("WARNING: Service %s not deleted\n"), C0C_SERVICE);
  ...
  do {
    ...
    if (err == ERROR_SUCCESS) {
      Trace(_T("Deleted EventLog %s\n"), C0C_SERVICE);
      notDeleted = FALSE;
    }
    else
    if (err == ERROR_FILE_NOT_FOUND) {
      Trace(_T("EventLog %s not installed\n"), C0C_SERVICE);
      notDeleted = FALSE;
    }
    else {
      res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegDeleteKey(%s)"), C0C_REGKEY_EVENTLOG);
    }
  } while (res == IDTRYAGAIN);

  if (notDeleted)
    Trace(_T("WARNING: Key %s not deleted\n"), C0C_REGKEY_EVENTLOG);
  ...
  do {
    ...
    if (hKey != INVALID_HANDLE_VALUE) {
      do {
        ...
        if (err != ERROR_SUCCESS) {
          ...
          if (err == ERROR_SUCCESS) {
            for (;;) {
              ...
              if (err == ERROR_SUCCESS)
                Trace(_T("Deleted Class subkey %s\\%s\n"), C0C_CNCCLASS_GUID, subKey);
              else
              if (err != ERROR_FILE_NOT_FOUND) {
                ShowError(MB_OK|MB_ICONWARNING, err, _T("RegDeleteKey(%s\\%s)"), C0C_CNCCLASS_GUID, subKey);
                break;
              }
            }
            ...
            if (err != ERROR_SUCCESS)
              ShowError(MB_OK|MB_ICONWARNING, err, _T("RegCloseKey()"));
          }
        ...
        if (err == ERROR_SUCCESS) {
          Trace(_T("Deleted Class %s\n"), C0C_CNCCLASS_GUID);
          notDeleted = FALSE;
        }
        else
        if (err == ERROR_FILE_NOT_FOUND) {
          Trace(_T("Class %s not installed\n"), C0C_CNCCLASS_GUID);
          notDeleted = FALSE;
        }
        else {
          res = ShowError(MB_CANCELTRYCONTINUE, err, _T("RegDeleteKey(%s)"), C0C_CNCCLASS_GUID);
        }
      } while (res == IDTRYAGAIN);
      ...
      res = ShowLastError(MB_CANCELTRYCONTINUE, _T("SetupDiOpenClassRegKey(NULL)"));
    }
  } while (res == IDTRYAGAIN);

  if (notDeleted)
    Trace(_T("WARNING: Class %s not deleted\n"), C0C_CNCCLASS_GUID);
    ...
    const InfFile::InfFileUninstall *pInfFileUninstallList)
{
  ...
  const TCHAR **ppOemPathExcludeList = NULL;
  int numInfFiles = 0;
      ...
      pInfFileInstall++)
  {
    ...
    if (!pNewInfFiles) {
      ShowError(MB_OK|MB_ICONSTOP, ERROR_NOT_ENOUGH_MEMORY, _T("LocalAlloc(%lu)"), (numInfFiles + 1) * sizeof(InfFile *));
      ok = FALSE;
      goto end;
    }
  ...
  if (pInfFiles != NULL) {
    ppOemPathExcludeList = (const TCHAR **)LocalAlloc(LPTR, (numInfFiles + 1) * sizeof(const TCHAR *));

    if (!ppOemPathExcludeList) {
      ShowError(MB_OK|MB_ICONSTOP, ERROR_NOT_ENOUGH_MEMORY, _T("LocalAlloc(%lu)"), (numInfFiles + 1) * sizeof(const TCHAR *));
      ok = FALSE;
      goto end;
    }
...
///////////////////////////////////////////////////////////////
bool ShowBusyNames(const TCHAR *pPattern)
{
  TCHAR *pPatternUp;
  ...
  for (int i = 0 ; i < 2 ; i++) {
    ...
    for (DWORD size = 1024 ; size >= 1024 ; size += 1024) {
      ...
      if (!pBuf)
        pBufNew = (TCHAR *)LocalAlloc(LPTR, size);
      else
        pBufNew = (TCHAR *)LocalReAlloc(pBuf, size, LMEM_ZEROINIT|LMEM_MOVEABLE);
      ...
      if (!pBufNew) {
        ...
        if (ShowError(MB_OKCANCEL, ERROR_NOT_ENOUGH_MEMORY, _T("LocalAlloc(%lu)"), (unsigned long)size) != IDOK) {
          if (pNames)
            LocalFree(pNames);
      ...
      if (!res) {
        ...
        if (ShowLastError(MB_OKCANCEL,
            i != 0 ? _T("QueryDosDevice()") : _T("ComDbNames()")) != IDOK)
        {
          if (pNames)
            LocalFree(pNames);
    ...
    if (pBuf) {
      for (TCHAR *pName = pBuf ; *pName ; pName += lstrlen(pName) + 1) {
        static const TCHAR strangeChars[] = _T(":#&$?!{}()[]/\\ \t\r\n");

        if (_tcspbrk(pName, strangeChars))
        ...
        if (!pNames)
          pNamesNew = (TCHAR *)LocalAlloc(LPTR, sizeNamesNew);
        else
          pNamesNew = (TCHAR *)LocalReAlloc(pNames, sizeNamesNew, LMEM_ZEROINIT|LMEM_MOVEABLE);

        if (!pNamesNew) {
          if (ShowError(MB_OKCANCEL, ERROR_NOT_ENOUGH_MEMORY, _T("LocalAlloc(%lu)"), (unsigned long)sizeNamesNew) != IDOK) {
            if (pNames)
              LocalFree(pNames);
  ...
  if (pNames) {
    for (TCHAR *pName = pNames ; *pName ; pName += lstrlen(pName) + 1)
      Trace(_T("%s\n"), pName);
...
///////////////////////////////////////////////////////////////
void Help(const TCHAR *pProgName)
{
  SetTitle(C0C_SETUP_TITLE _T(" (HELP)"));
  ...
  ConsoleWrite(
      _T("Usage:\n"
    "  %s%s[options] <command>\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("\n"
    "Options:\n"
  ...
  ConsoleWrite(
      _T("  change <portid> <prms>       - set parameters <prms> for port with\n"
    "                                 identifier <portid>\n"
  ...
  const TCHAR *pStr = PortParameters::GetHelp();

  while (*pStr) {
    TCHAR buf[100];

    SNPRINTF(buf, sizeof(buf)/sizeof(buf[0]), _T("%s"), pStr);
    pStr += lstrlen(buf);

    ConsoleWrite(_T("%s"), buf);
  }
  ...
  ConsoleWrite(
    ...
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("  %s%sinstall PortName=COM2 PortName=COM4\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("  %s%sinstall PortName=COM5,EmuBR=yes,EmuOverrun=yes -\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("  %s%schange ") C0C_PREF_PORT_NAME_A _T("0 EmuBR=yes,EmuOverrun=yes\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("  %s%schange ") C0C_PREF_PORT_NAME_A _T("0 PortName=-\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
  ConsoleWrite(
      _T("  %s%slist\n")
    , pProgName, (pProgName && *pProgName) ? _T(" ") : _T(""));
...
int Main(int argc, const TCHAR* argv[])
{
  ...
  while (argc > 1) {
    ...
    if (!_tcscmp(argv[1], _T("--output")) && argc > 2) {
    ...
    if (!_tcscmp(argv[1], _T("--wait")) && argc > 2) {
      int num;

      if (!StrToInt(argv[2], &num) || num < 0) {
        ConsoleWrite(_T("Invalid option %s %s\n"), argv[1], argv[2]);
        return 1;
      }
    ...
    if (!_tcscmp(argv[1], _T("--detail-prms"))) {
      detailPrms = TRUE;
      argv[1] = argv[0];
      argv++;
      argc--;
    ...
    if (!_tcscmp(argv[1], _T("--no-update"))) {
      no_update = TRUE;
      argv[1] = argv[0];
      argv++;
      argc--;
    ...
    if (!_tcscmp(argv[1], _T("--no-update-fnames"))) {
      no_update_fnames = TRUE;
      argv[1] = argv[0];
      argv++;
      argc--;
    ...
    if (!_tcscmp(argv[1], _T("--show-fnames"))) {
      show_fnames = TRUE;
      argv[1] = argv[0];
      argv++;
      argc--;
      ...
      ConsoleWrite(_T("Invalid option %s\n"), argv[1]);
      return 1;
    }
  }
  ...
  if (argc == 2 && !lstrcmpi(argv[1], _T("help"))) {
    Help(argv[0]);
    return Complete(TRUE);
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("quit"))) {
    return Complete(TRUE);
  }
  else
  if (argc == 3 && !lstrcmpi(argv[1], _T("busynames"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (BUSY NAMES)"));
    return Complete(ShowBusyNames(argv[2]));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("list"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (LIST)"));
    return Complete(Change(NULL, NULL));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("updatefnames"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (UPDATE FRIENDLY NAMES)"));
    return Complete(UpdateFriendlyNames(TRUE));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("listfnames"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (LIST FRIENDLY NAMES)"));
    return Complete(UpdateFriendlyNames(FALSE));
  }
  else
  if (argc == 4 && !lstrcmpi(argv[1], _T("change"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (CHANGE)"));
    return Complete(Change(argv[2], argv[3]));
  }
  else
  if (argc == 3 && !lstrcmpi(argv[1], _T("remove"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (REMOVE)"));
  ...
  if (argc == 3 && !lstrcmpi(argv[1], _T("disable"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (DISABLE)"));

    if (!lstrcmpi(argv[2], _T("all")))
      return Complete(Disable());
  }
  else
  if (argc == 3 && !lstrcmpi(argv[1], _T("enable"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (ENABLE)"));

    if (!lstrcmpi(argv[2], _T("all")))
      return Complete(Enable());
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("reload"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (RELOAD)"));
  ...
  if (argc == 2 && !lstrcmpi(argv[1], _T("preinstall"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (PREINSTALL)"));
    return Complete(Preinstall(infFileInstallList));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("uninstall"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (UNINSTALL)"));
    return Complete(Uninstall(infFileInstallList, infFileUnnstallList));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("infclean"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (INF CLEAN)"));
    return Complete(InfClean(infFileInstallList, infFileUnnstallList));
  }
  else
  if (argc == 2 && !lstrcmpi(argv[1], _T("update"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (UPDATE)"));
    return Complete(Update(infFileInstallList));
  }

  InfFile infFile(C0C_INF_NAME, C0C_INF_NAME);

  const TCHAR *pPath = infFile.Path();

  if (!pPath)
    return 1;

  if (argc == 2 && !lstrcmpi(argv[1], _T("install"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (INSTALL)"));
    return Complete(Install(pPath));
  }
  else
  if (argc == 4 && !lstrcmpi(argv[1], _T("install"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (INSTALL)"));
    return Complete(Install(pPath, argv[2], argv[3], -1));
  }
  else
  if (argc == 5 && !lstrcmpi(argv[1], _T("install"))) {
    SetTitle(C0C_SETUP_TITLE _T(" (INSTALL)"));
  ...
  ConsoleWrite(_T("Invalid command\n"));

  return 1;
}
///////////////////////////////////////////////////////////////
static int ParseCmd(TCHAR *pCmd, const TCHAR* argv[], int sizeArgv)
{
  int argc;
  ...
  for (TCHAR *pArg = STRTOK_R(pCmd, _T(" \t\r\n"), &pSave) ; pArg ; pArg = STRTOK_R(NULL, _T(" \t\r\n"), &pSave)) {
    if ((argc + 2) > sizeArgv)
      break;
...
///////////////////////////////////////////////////////////////
int CALLBACK Main(const TCHAR *pProgName, const TCHAR *pCmdLine)
{
  SetTitle(C0C_SETUP_TITLE);

  TCHAR cmd[1024];

  SNPRINTF(cmd, sizeof(cmd)/sizeof(cmd[0]), _T("%s"), pCmdLine);
  ...
  if (argc == 1) {
    ConsoleWrite(_T("Enter 'help' to get info about usage of ") C0C_SETUP_TITLE _T(".\n\n"));
    ...
    for (;;) {
      ...
      ConsoleWriteRead(cmd, sizeof(cmd)/sizeof(cmd[0]), _T("command> "));
...
///////////////////////////////////////////////////////////////
int CALLBACK RunDll(HWND /*hWnd*/, HINSTANCE /*hInst*/, LPTSTR pCmdLine, int /*nCmdShow*/)
{
  int res = Main(_T("rundll32 setup,RunDll"), pCmdLine);

  if (!GetOutputFile()) {
    TCHAR buf[10];

    ConsoleWriteRead(buf, sizeof(buf)/sizeof(buf[0]), _T("\nPress <RETURN> to continue\n"));
  }
