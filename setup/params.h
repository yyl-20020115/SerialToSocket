...
class PortParameters {
  public:
    PortParameters(const TCHAR *pService, const TCHAR *pPhPortName);

    void Init();
    LONG Load();
    LONG Save();
    bool InitRealPortName(const TCHAR *pRealPortName = _T(""));
    bool ParseParametersStr(const TCHAR *pParameters);
    bool FillParametersStr(TCHAR *pParameters, int size, bool detail);
    bool FillPortName(TCHAR *pPortName, int size);
    bool FillRealPortName(TCHAR *pRealPortName, int size);
    bool Changed() const { return maskChanged != 0; }

    static const TCHAR *GetHelp();

  protected:
    bool FillParametersKey(TCHAR *pRegKey, int size);
    DWORD *GetDwPtr(DWORD bit);
    void LoadDw(HKEY hKey, DWORD bit);
    LONG SaveDw(HKEY hKey, DWORD bit);

    bool SetPortName(const TCHAR *pNewPortName);
    bool SetRealPortName(const TCHAR *pNewRealPortName);
    bool SetFlag(const TCHAR *pNewVal, DWORD bit);
    bool SetPin(const TCHAR *pNewVal, DWORD bit);
    bool SetProbability(const TCHAR *pNewVal, DWORD bit);
    bool SetUnsigned(const TCHAR *pNewVal, DWORD bit);
    bool SetBit(const TCHAR *pVal, const Bit &bit);

    DWORD maskChanged;
    DWORD maskExplicit;
    TCHAR portName[20];
    TCHAR realPortName[20];
    DWORD emuBR;
    DWORD emuOverrun;
    DWORD plugInMode;
    DWORD exclusiveMode;
