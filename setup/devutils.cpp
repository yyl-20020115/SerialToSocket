...
///////////////////////////////////////////////////////////////
static const TCHAR *SetStr(TCHAR **ppDst, const TCHAR *pSrc)
{
  if (*ppDst) {
    LocalFree(*ppDst);
    *ppDst = NULL;
  ...
  if (pSrc) {
    ...
    *ppDst = (TCHAR *)LocalAlloc(LPTR, len * sizeof(pSrc[0]));

    if (*ppDst) {
      SNPRINTF(*ppDst, len, _T("%s"), pSrc);
    } else {
      SetLastError(ERROR_NOT_ENOUGH_MEMORY);
      ShowLastError(MB_OK|MB_ICONSTOP, _T("LocalAlloc(%lu)"), (unsigned long)(len * sizeof(pSrc[0])));
    }
  }
...
///////////////////////////////////////////////////////////////
const TCHAR *DevProperties::DevId(const TCHAR *_pDevId)
{
  return SetStr(&pDevId, _pDevId);
}

const TCHAR *DevProperties::PhObjName(const TCHAR *_pPhObjName)
{
  return SetStr(&pPhObjName, _pPhObjName);
}

const TCHAR *DevProperties::Location(const TCHAR *_pLocation)
{
    ...
    PEnumParams pEnumParams)
{
  ...
  if (hDevInfo == INVALID_HANDLE_VALUE) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiGetClassDevs()"));
    return IDCANCEL;
  }
  ...
  for (int i = 0 ; SetupDiEnumDeviceInfo(hDevInfo, i, &devInfoData) ; i++) {
    ...
    if (!SetupDiGetDeviceRegistryProperty(hDevInfo, &devInfoData, SPDRP_HARDWAREID, NULL, (PBYTE)hardwareId, sizeof(hardwareId), NULL)) {
      ...
      SNPRINTF(hardwareId, sizeof(hardwareId)/sizeof(hardwareId[0]), _T("UNKNOWN HARDWAREID") _T("\0"));
    }

    if (!pFilter(hardwareId))
      continue;

    const TCHAR *pHardwareId = hardwareId;
...
static bool UpdateRebootRequired(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDevInfoData, BOOL *pRebootRequired)
{
  ...
  *pRebootRequired =
  ...
  //if (*pRebootRequired)
  //  Trace(_T("Enumerated status=0x%lX problem=0x%lX\n"), status, problem);
  ...
  if (!SetupDiGetDeviceInstallParams(hDevInfo, pDevInfoData, &installParams)) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiGetDeviceInstallParams()"));
    return FALSE;
  }
  ...
  *pRebootRequired = (installParams.Flags & (DI_NEEDREBOOT|DI_NEEDRESTART)) ? TRUE : FALSE;
  ...
  //if (*pRebootRequired)
  //  Trace(_T("Enumerated Flags=0x%lX\n"), installParams.Flags);
...
static bool ChangeState(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDevInfoData, DWORD stateChange)
{
  ...
  if (!SetupDiSetClassInstallParams(hDevInfo, pDevInfoData, (SP_CLASSINSTALL_HEADER *)&propChangeParams, sizeof(propChangeParams))) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiSetClassInstallParams()"));
    return FALSE;
  }

  if (!SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, hDevInfo, pDevInfoData)) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiCallClassInstaller(DIF_PROPERTYCHANGE)"));
    return FALSE;
  }
    ...
    Stack *pDevPropertiesStack)
{
  ...
  if (!rebootRequired) {
    ...
    if (rebootRequired) {
      Trace(_T("Can't stop device %s %s %s\n"),
            pDevProperties->Location(),
            pDevProperties->DevId(),
            pDevProperties->PhObjName());
...
static int RestartDevice(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDevInfoData, PDevParams pDevParams)
{
  ...
  if (rebootRequired) {
    Trace(_T("Can't reastart device %s %s %s\n"),
          pDevParams->devProperties.Location(),
          pDevParams->devProperties.DevId(),
          pDevParams->devProperties.PhObjName());
    ...
    if (pDevParams->pEnumParams->pRebootRequired && !*pDevParams->pEnumParams->pRebootRequired) {
      ...
      res = ShowMsg(MB_CANCELTRYCONTINUE,
                        _T("Can't reastart device %s %s %s.\n"
                        "Close application that use this device and Try Again.\n"
                        "Or Continue and then reboot system.\n"),
                        pDevParams->devProperties.Location(),
                        pDevParams->devProperties.DevId(),
    ...
    BOOL *pRebootRequired)
{
  ...
  if (updateErr != ERROR_SUCCESS) {
    if (updateErr == ERROR_FILE_NOT_FOUND) {
      ...
      if (err == ERROR_FILE_NOT_FOUND) {
        int res = ShowMsg(MB_CANCELTRYCONTINUE,
                          _T("Can't update driver. Possible it's because your Windows registry is corrupted and\n"
        ...
        if (res == IDCONTINUE) {
          ...
          if (err == ERROR_SUCCESS) {
            ...
            ShowLastError(MB_OK|MB_ICONSTOP, _T("RegCreateKeyEx()"));
            return IDCANCEL;
          }
        }
    ...
    return ShowError(MB_CANCELTRYCONTINUE, updateErr,
                     _T("UpdateDriverForPlugAndPlayDevices(\"%s\", \"%s\", 0x%lX)"),
    ...
    BOOL *pRebootRequired)
{
  ...
  if (!SetupDiGetINFClass(pInfFilePath, &classGUID, className, sizeof(className)/sizeof(className[0]), 0)) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiGetINFClass(%s)"), pInfFilePath);
    return IDCANCEL;
  }
  ...
  if (hDevInfo == INVALID_HANDLE_VALUE) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiCreateDeviceInfoList()"));
    return IDCANCEL;
  }
  ...
  if (!pDevInstID) {
    if (StrCmpNI(pDevId, _T("root\\"), 5) == 0) {
      /*
  ...
  if (StrChr(pDevInstID, '\\')) {
    ...
    if (res != IDCONTINUE && GetLastError() == ERROR_DEVINST_ALREADY_EXISTS) {
      TCHAR *pTmpDevInstID = NULL;

      if (SetStr(&pTmpDevInstID, pDevInstID)) {
        TCHAR *pSave;
        TCHAR *p;

        p = STRTOK_R(pTmpDevInstID, _T("\\"), &pSave);

        if (p && !lstrcmp(p, REGSTR_KEY_ROOTENUM)) {
          p = STRTOK_R(NULL, _T("\\"), &pSave);
  ...
  if (res != IDCONTINUE) {
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiCreateDeviceInfo()"));
    goto exit1;
  }

  TCHAR hardwareId[MAX_DEVICE_ID_LEN + 1 + 1];

  SNPRINTF(hardwareId, sizeof(hardwareId)/sizeof(hardwareId[0]) - 1, _T("%s"), pDevId);
  ...
  {
    res = IDCANCEL;
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiSetDeviceRegistryProperty()"));
    goto exit1;
  }

  if (!SetupDiCallClassInstaller(DIF_REGISTERDEVICE, hDevInfo, &devInfoData)) {
    res = IDCANCEL;
    ShowLastError(MB_OK|MB_ICONSTOP, _T("SetupDiCallClassInstaller()"));
    goto exit1;
  }
...
exit2:
  ...
  if (res != IDCONTINUE) {
    if (!SetupDiCallClassInstaller(DIF_REMOVE, hDevInfo, &devInfoData))
      ShowLastError(MB_OK|MB_ICONWARNING, __T("SetupDiCallClassInstaller()"));
  }
...
bool WaitNoPendingInstallEvents(int timeLimit, bool repeate)
{
  ...
  for (int count = 0 ;;) {
    ...
    if (res != WAIT_TIMEOUT) {
      ShowLastError(MB_OK|MB_ICONWARNING, _T("CMP_WaitNoPendingInstallEvents()"));
      if (inTrace)
        Trace(_T(" FAIL\n"));
      return FALSE;
    }
    ...
    if (timeLimit != -1 && timeElapsed >= DWORD(timeLimit * 1000)) {
      ...
      if (!Silent() && repeate) {
        if (ShowMsg(MB_YESNO,
            _T("The device installation activities are still pending.\n"
            "Continue to wait?\n")) == IDYES)
        {
    ...
    if (!inTrace) {
      if (timeLimit != -1)
        Trace(_T("Waiting for no pending device installation activities (%u secs) "), (unsigned)timeLimit);
      else
        Trace(_T("Waiting for no pending device installation activities (perpetually) "));
